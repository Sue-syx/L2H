...
def run():
    args = load_config()
    logger.add(
        os.path.join('logs', '{}_model_{}_codelength_{}_mu_{}_nu_{}_eta1_'
    ...
    for code_length in args.code_length:
        ...
        torch.save(checkpoint,
                   os.path.join('checkpoints', '{}_model_{}_codelength_'
                                               '{}_mu_{}_nu_{}_eta1_{}_eta2_{}_eta3_{}_'
                                               'topk_{}_map_{:.4f}.pt'
                                .format(args.dataset, args.arch, code_length, args.mu, args.nu,
                                        args.eta1, args.eta2, args.eta3, args.topk, checkpoint['map'])))
...
def load_config():
    ...
    parser.add_argument('--eta1', default=2, type=float,
                        help='Hyper-parameter.(default: 2)')
    parser.add_argument('--eta2', default=10, type=float,
                        help='Hyper-parameter.(default: 10)')
    parser.add_argument('--eta3', default=10, type=float,
                        help='Hyper-parameter.(default: 10)')
