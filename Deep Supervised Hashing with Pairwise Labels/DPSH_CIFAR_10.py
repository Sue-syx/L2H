...
def Logtrick(x, use_gpu):
    if use_gpu:
        lt = torch.log(1 + torch.exp(-torch.abs(x))) + \
             torch.max(x, Variable(torch.FloatTensor([0.]).cuda()))
    else:
        lt = torch.log(1 + torch.exp(-torch.abs(x))) + \
             torch.max(x, Variable(torch.FloatTensor([0.])))
    return lt
...
def Totloss(U, B, Sim, lamda, num_train):
    ...
    l1 = (- theta * Sim + Logtrick(Variable(theta), True).data).sum()
    l2 = (U - B).pow(2).sum()
    l = l1 + lamda * l2
    return l, l1, l2, t1
...
def DPSH_algo(bit, param, gpu_ind=0):
    ...
    for epoch in range(epochs):
        ...
        print('[Train Phase][Epoch: %3d/%3d][Loss: %3.5f]'
              % (epoch + 1, epochs, epoch_loss / len(train_loader)), end=' ')
...
if __name__ == '__main__':
    ...
    lamda = 50
    gpu_ind = 0
    filename = ('log/DPSH_' + str(bit) + 'bits_NUS-WIDE_' +
